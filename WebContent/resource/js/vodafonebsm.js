...
var unenrollservices=[]; 
	...
	$(function() {
		...
		function PopulateAllServiceBoxes(page,statusFilter, pattern){
			...
			$.get('/PopulateServicesBoxes?page='+page+'&statusFilter='+statusFilter+'&spattern=' + pattern,function(responseJson) {
				if(listgridvalue == 0){
					...
					$.each(responseJson, function(key,value){
						if(listgridvalue == 0){
							...
							var divinner = $("<div></div>")
								.addClass("inner").css("display","none").appendTo(divbox);
							...
							a.appendTo(divbox);
							var h3 = $("<h3></h3>").text(ametric + '%').css("display","table-cell").css("padding-left","10px").appendTo(a);
							var p = $("<p></p>").text(value["service_instance_name"]).css("display","table-cell").css("padding-left","20px").appendTo(a);
...
/*	                     between: {
        ...
        //DataSource-KPI-User-Role-Group-Report Delete and Delete-Confirmation Model Close
	    ...
	    function Paginate(statusFilter, pattern){
	    	if(statusFilter != undefined && statusFilter.length > 0){
	    		...
	    		var xurl = '/ServiceStatOps?p=getServiceCount&statusFilter='+statusFilter+'&spattern=' + pattern;
		    	$.ajax({
		    		cache: false,
		    		url:xurl,
		    		success:function(data) {
	...
	function showAllParentDivRSB(d,data){
		$.ajax({
			...
			success:function(edata) {
				...
				var netr = $("<tr></tr>");
				$("<td class=\"bg-primary\" style=\"width:200px;\" nowrap><label nowrap>Node Name</label></td><td style=\"width:100%;\" nowrap><label nowrap>" + d.name + "</label></td>").appendTo(netr);
				netr.appendTo(netbl);
				...
				var citr = $("<tr></tr>");
				$("<td class=\"bg-primary\" style=\"width:200px;\" nowrap><label nowrap>CI Type</label></td><td nowrap style=\"width:100%;\"><label nowrap>" + d.citype + "</label></td>").appendTo(citr);
				citr.appendTo(citbl);
				...
				var comptd1 = $('<td nowrap class="bg-primary" style=\"width:200px;\"><label nowrap>Comp.Model</label></td>');
				...
				$("<td nowrap style=\"width:200px;\"><label nowrap>Output Rules</label></td><td nowrap style=\"width:100%;\"><label nowrap>Child Weight</label></td>").appendTo(outputheader);
	...
	function kpiEditFunc(){	
		...
		$("#loader").attr('class', 'loader');
		console.log('/Kpi?p=saveKPI&name=' + kpiname+ '&sid=' + sid + '&query=' + kpiquery + '&interval=' + interval + '&timeunit=' + timeunit+ '&thresholdMarginalRule=' + thrsmarrule+ '&thresholdBadRule=' + thrsbadrule+ '&dsid='+dsid);
		$.ajax({
			cache: false,
			url:'/Kpi?p=saveKPI&name=' + kpiname+ '&sid=' + sid + '&query=' + kpiquery + '&interval=' + interval + '&timeunit=' + timeunit+ '&thresholdMarginalRule=' + thrsmarrule+ '&thresholdBadRule=' + thrsbadrule+ '&dsid='+dsid,
			success:function(data) {
				$("#loader").attr('class', 'loader collapse');
