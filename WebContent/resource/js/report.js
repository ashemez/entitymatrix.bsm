...
function showReport(tbodyID, _sid){
	...
	$.ajax({
		...
		success:function(data) {
			if(data === "false"){
				$('#createRptFromQueryBtn').attr('style', 'display:none;');
			} else {
				$('#createRptFromQueryBtn').attr('style', '');
			}
		}
	});
	...
	$.ajax({
		cache: false,
		url:'/Report?p=' + p + '&sid=' + _sid + '&queryType=' + queryType + '&startingDate=' + startingDate + '&endingDate=' + endingDate + '&getOutageDetails=' + getOutageDetails,
		success:function(data) {
			$.each(JSON.parse(data), function(key,value) {
				  ...
				  var innerTD = $("<td class=\"\" style=\"border-spacing:0;padding:3 0 0 0;\" colspan=\"5\" id=\"" + trInd + "td" + value.service_instance_id + "\"></td>").text("");
...
function RefreshReport(){
	...
	url = '/ReportViewer.jsp?p=' + p + '&report_id=' + report_id + '&sid=' + _ssid + '&queryType=' + queryType + '&startingDate=' + startingDate + '&endingDate=' + endingDate + '&getOutageDetails=' + getOutageDetails
	//ChangeUrl('Service Manager', url);
}
...
function ShowReportList(){
	...
	$.ajax({
		...
		success:function(data) {
			if(data === "false"){
				$('#createRptBtn').attr('style', 'display:none;');
			} else {
				$('#createRptBtn').attr('style', '');
			}
		}
	});
...
function ShowReportJobList(){
	...
	$.ajax({
		...
		success:function(data) {
			if(data === "false"){
				$('#createRptJobBtn').attr('style', 'display:none;');
			} else {
				$('#createRptJobBtn').attr('style', '');
			}
		}
	});
